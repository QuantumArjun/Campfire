...
const Setup = ({ location }) => {
  ...
  const genRoom = () => {
    ...
    while(bool1 && count < 10) {
      var chosenRoom = adj_arr[Math.floor(Math.random() * adj_arr.length)].charAt(0).toUpperCase() + noun_arr[Math.floor(Math.random() * noun_arr.length)].charAt(0).toUpperCase();
